...
import { Incident, IncidentStatus, IncidentType } from '../store/incidents.models';
import {
  selectError,
  selectStatusFilter,
  selectTypeFilter,
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, AsyncPipe, FiltersComponent, IncidentListComponent, MapComponent],
...
export class UrbanMapPageComponent implements OnInit {
  ...
  readonly incidents$: Observable<readonly Incident[]> = this.store.select(selectFilteredIncidents);
